...
- **Agentic Code Generation:** Translates user prompts into clean Angular code (`HTML`, `TS`, `CSS`) and strips conversational filler.
- **Design System Enforcement:** Injects a strict JSON design token schema (colors, fonts, radii) into the LLM context.
- **Automated Validation & Self-Correction:** An integrated Linter-Agent intercepts the raw LLM output, validates syntax and CSS variables via AST parsing, and automatically triggers a self-correction loop if the model hallucinates.
- **Guardrails AI Security:** A robust input firewall that instantly blocks Prompt Injections (Jailbreaks), Profanity, Personally Identifiable Information (PII), and leaked AWS Secrets.
- **Live Edit Mode:** A stunning glassmorphism UI that streams tokens in real-time. Features a manual "Edit Code" toggle that unlocks the editor, allowing developers to manually tweak the generated Angular code and hit "Render" to live-update the UI without confusing the LLM's history state.
...
- **Infrastructure:** Redis, Docker, Kubernetes, AWS SageMaker
